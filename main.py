...
{RESET}
          ...
          {YELLOW}>>        CHUNZA CORE: FULL ARMED           <<{RESET}
    """
    print(banner)
...
async def on_raw_reaction_add(payload):
    ...
    if role:
        ...
        print(f"{GREEN}[VERIFY] Role Granted: {payload.member.name}{RESET}")
...
async def on_raw_reaction_remove(payload):
    ...
    if role:
        ...
        print(f"{RED}[VERIFY] Role Revoked: {member.name}{RESET}")
...
async def on_ready():
    ...
    print(f"{GREEN}[+] MONITORING GUILD: {TARGET_GUILD_ID}{RESET}")
...
async def start_vote(ctx, *, topic=None):
    ...
    try:
        ...
        sec = int(t_str.replace('분',''))*60 if '분' in t_str else int(t_str.replace('일',''))*86400
...
app = Flask('')
@app.route('/')
def home(): return "CHUNZA v1.6.0 Online"
